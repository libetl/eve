...
/**
 * Commande permettant d'afficher les groupes avec toutes les commandes permises
 * pour chaque groupe
 * 
 * @author benychou
 * 
 */
public class CommandeAfficherGroupesAdmin extends AbstractCommande {

  public CommandeAfficherGroupesAdmin () {
    super ();
  }

  public CommandeAfficherGroupesAdmin (final Boolean b) {
    super (b);
  }
  ...
  public Object executer () {
    ...
    final List<Commandes> lcSu = Commandes.staticInstance (gt).getParNiveau (1);

    m.mettreAJour ();

    resultat[0] = GroupeContrainte.staticInstance (gt).requete ();
    Collections.sort ((List<GroupeContrainte>) resultat[0]);

    resultat[1] = VueGroupes.vue (lg.iterator (), abt.getP (), lcSu);
    ...
    return new RedirectResolution ("/controleurs/" + //$NON-NLS-1$
        Commandes.staticInstance (m.getGT ())
            .getParNom ("AfficherGroupesAdmin").getPartie ().getPartie () + //$NON-NLS-1$
        "/AfficherGroupesAdmin", true); //$NON-NLS-1$
  }
}
